...
import com.framework.pay.core.client.util.LocalDateTimeUtils;
...
public class WxBarPayClient extends AbstractWxPayClient {
    ...
    protected PayOrderRespDTO doUnifiedOrderV2(PayOrderUnifiedReqDTO reqDTO) throws WxPayException {
        ...
        for (int i = 1; i < Byte.MAX_VALUE; i++) {
            try {
                ...
                PayOrderRespDTO payOrderResp = PayOrderRespDTO.successOf(response.getTransactionId(), response.getOpenid(), parseDateV2(response.getTimeEnd()),
                    response.getOutTradeNo(), response);
                payOrderResp.setDisplayMode(PayOrderDisplayModeEnum.BAR_CODE.getMode());
                return payOrderResp;
                ...
                log.info("[doUnifiedOrderV2][发起微信 Bar 支付第({})失败，等待下一轮重试，请求({})，响应({})]", i,
                        JSONUtil.toJsonStr(request), ex.getMessage());
    ...
    static String getAuthCode(PayOrderUnifiedReqDTO reqDTO) {
        ...
        if (StrUtil.isEmpty(authCode)) {
            throw new PayException(401, "支付请求的 authCode 不能为空！");
        }
        return authCode;
    }
